 ...
 * what the provider schema takes. Models the catalog hides (`visibility`) or
 * excludes from the API are dropped, and reasoning levels the harness does not
 * know (for example `ultra`) are left out rather than mistranslated.
 *
 * A model is also dropped when the catalog marks it as superseded — its
 * `upgrade.model` names another listable model — so releasing a successor (for
 * example `gpt-6-sol`) retires its predecessors from the selector. The
 * `exclude` option covers models OpenAI still lists but this deployment does
 * not want offered.
 *
 * @param payload - the decoded `/backend-api/codex/models` response.
 * @param options - optional `exclude` list of model ids to hide.
 * @returns one `models` entry per served Codex model, in catalog order.
 */
export function codexModelCatalogEntries(payload, options = {}) {
  ...
  const entries = [];
  for (const [id, model] of listed) {
    if (excluded.has(id)) continue;
    const upgrade =
      typeof model.upgrade?.model === "string" ? model.upgrade.model.trim() : "";
    if (upgrade.length > 0 && listed.has(upgrade)) continue;
    const entry = {
      id,
      name:
